...
$admin_id = $_SESSION['admin_id'] ?? 0; // Get admin_id from session or default to 0
$vehicle = null;
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['payment_method'])) {
    ...
    // Get vehicle price
    $price_stmt = $conn->prepare("SELECT price FROM Vehicles WHERE vehicle_id = ?");
    $price_stmt->bind_param("i", $vehicle_id);
    $price_stmt->execute();
    $price_result = $price_stmt->get_result();
    $vehicle_data = $price_result->fetch_assoc();
    $price_stmt->close();
    
    if ($vehicle_data) {
        // Insert sale with admin_id
        $sale_sql = "INSERT INTO VehicleSales (customer_id, admin_id, vehicle_id, sale_price, payment_method) VALUES (?, ?, ?, ?, ?)";
        $stmt = $conn->prepare($sale_sql);
        $stmt->bind_param("iiids", $customer_id, $admin_id, $vehicle_id, $vehicle_data['price'], $payment_method);
        $stmt->execute();
        $stmt->close();
        ...
        // Check stock and mark as sold if needed
...
// Get vehicle details if ID is provided
