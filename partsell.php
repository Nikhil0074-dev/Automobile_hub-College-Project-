...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['payment_method'])) {
    ...
    // Get part price
    $price_stmt = $conn->prepare("SELECT price FROM Parts WHERE part_id = ?");
    $price_stmt->bind_param("i", $part_id);
    $price_stmt->execute();
    ...
    if ($part_data) {
        ...
        $sale_sql = "INSERT INTO PartSales (customer_id, admin_id, part_id, sale_price, payment_method) VALUES (?, ?, ?, ?, ?)";
        $stmt = $conn->prepare($sale_sql);
        $stmt->bind_param("iiids", $customer_id, $admin_id, $part_id, $part_data['price'], $payment_method);
        $stmt->execute();
        $stmt->close();
...
// Get part details if ID is provided
