    ...
    if (empty($vehicle_no)) {
        ...
        if ($count > 0) {
            ...
            if ($sale_price) {
                ...
                $insert_sql = "INSERT INTO InsuranceSale (customer_id, insurance_id, vehicle_no, sale_price, payment_method) 
                               VALUES (?, ?, ?, ?, ?)";
                $insert_stmt = $conn->prepare($insert_sql);
                $insert_stmt->bind_param("iisss", $customer_id, $insurance_id, $vehicle_no, $sale_price, $payment_method);
